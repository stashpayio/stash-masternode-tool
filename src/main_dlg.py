...
class MainWindow(QMainWindow, WndUtils, ui_main_dlg.Ui_MainWindow):
    ...
    def setupUi(self):
        ...
        self.stashd_intf.initialize(self.app_config)
    ...
    def load_configuration_from_file(self, file_name: str, ask_save_changes = True,
        ...
        try:
            self.disconnect_hardware_wallet()
            stash_network_sav = self.app_config.stash_network
            ...
            if file_name:
                ...
                self.update_config_files_mru_menu_items()
                if stash_network_sav != self.app_config.stash_network:
                    self.disconnect_hardware_wallet()
    ...
    def display_window_title(self):
        ...
        if self.app_config.stash_network == 'TESTNET':
    ...
    def on_action_open_settings_window_triggered(self):
        ...
        if res and dlg.get_is_modified():
            ...
            self.stashd_intf.reload_configuration()
            if stash_network_sav != self.app_config.stash_network or hw_type_sav != self.app_config.hw_type:
                self.disconnect_hardware_wallet()
                self.app_config.reset_network_dependent_dyn_params()
            self.display_window_title()
            self.update_edit_controls_state()
    ...
    def show_connection_initiated(self):
        ...
        self.set_status_text1('<b>RPC network status:</b> trying %s...' % self.stashd_intf.get_active_conn_description(), 'black')
    ...
    def show_connection_failed(self):
        ...
        self.set_status_text1('<b>RPC network status:</b> failed connection to %s' % self.stashd_intf.get_active_conn_description(), 'red')

    def show_connection_successful(self):
        """Shows status information after successful connetion to a Stash RPC node."""
        self.set_status_text1('<b>RPC network status:</b> OK (%s)' % self.stashd_intf.get_active_conn_description(), 'green')
    ...
    def connect_stash_network(self, wait_for_check_finish=False, call_on_check_finished=None):
            ...
            try:
                ...
                while not ctrl.finish:
                    ...
                    self.add_app_message(
                        DispMessage.STASH_NET_CONNECTION,
                        'Stashd is synchronizing: AssetID: %s, AssetName: %s' %
                        (str(mnsync.get('AssetID', '')), str(mnsync.get('AssetName', ''))), 'warn')
                    cond.wait(mtx, 5000)
                self.del_app_message(DispMessage.STASH_NET_CONNECTION)
            except Exception as e:
                self.is_stashd_syncing = False
                self.stashd_connection_ok = False
                self.add_app_message(DispMessage.STASH_NET_CONNECTION, str(e), 'error')
            finally:
                mtx.unlock()
        ...
        def connect_thread(ctrl):
            ...
            try:
                ...
                self.add_app_message(DispMessage.STASH_NET_CONNECTION, err, 'error')
        ...
        def connect_finished():
            ...
            self.connecting_to_stashd = False
            self.app_config.read_stash_network_app_params(self.stashd_intf)
                ...
                try:
                    ...
                    if self.app_config.stash_network == 'TESTNET':
                        ...
                        if self.app_config.hw_type in (HWType.trezor, HWType.keepkey):
                            try:
                                path = stash_utils.get_default_bip32_base_path(self.app_config.stash_network)
                                path += "/0'/0/0"
                                path_n = stash_utils.bip32_path_string_to_n(path)
                                addr = hw_intf.get_address(self.hw_session, path_n, False)
                                if addr and stash_utils.validate_address(addr, self.app_config.stash_network):
                            ...
                            addr = hw_intf.get_address(self.hw_session,
                                                       stash_utils.get_default_bip32_path(self.app_config.stash_network))
                            if stash_utils.validate_address(addr, self.app_config.stash_network):
                                found_testnet_support = False
    ...
    def get_masternode_status_description_thread(self, ctrl, masternode: MasternodeConfig):
        ...
        if self.stashd_connection_ok:
            ...
            self.stashd_intf.get_masternodelist('json', data_max_age=30)  # read new data from the network
                                                                         # every 30 seconds
            if collateral_id:
            ...
            block_height = self.stashd_intf.getblockcount()
            ...
            if mn_info:
                ...
                if mn_info.queue_position is not None:
                    ...
                    next_payout_ts = int(time.time()) + (mn_info.queue_position * 1.0 * 60)
            else:
                if dmn_tx_state:
                ...
                if masternode == self.cur_masternode:
                    ...
                    try:
                        if collateral_address:
                            collateral_bal = self.stashd_intf.getaddressbalance([collateral_address])
                        ...
                        if collateral_address != payout_address and payout_address:
                            payout_bal = self.stashd_intf.getaddressbalance([payout_address])
                    ...
                    if dmn_tx and not dmn_tx.get('confirmations'):
                        ...
                        if self.stashd_intf.is_protx_update_pending(self.cur_masternode.dmn_tx_hash):
    ...
    def update_registrar(self, show_upd_payout: bool, show_upd_operator: bool, show_upd_voting: bool):
        ...
        if self.cur_masternode:
            upd_dlg = upd_mn_registrar_dlg.UpdMnRegistrarDlg(
                self, self.app_config, self.stashd_intf, self.cur_masternode,

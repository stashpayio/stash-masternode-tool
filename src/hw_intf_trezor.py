...
log = logging.getLogger('smt.hw_intf_trezor')
...
def is_stash(coin):
    return coin["coin_name"].lower().startswith("stash")
...
def json_to_tx(coin, data):
    ...
    if is_stash(coin):
        ...
        if t.version == 3 and dip2_type != 0:
            ...
            t.extra_data = stash_utils.num_to_varint(data["extraPayloadSize"]) + bytes.fromhex(
                data["extraPayload"]
            )
...
class MyTxApiInsight(TxApi):
    ...
    def __init__(self, network, url, stashd_inf, cache_dir):
        TxApi.__init__(self, network)
        self.stashd_inf = stashd_inf
    ...
    def fetch_json(self, *path, **params):
        if path:
            if len(path) >= 2:
                if path[0] == 'tx':
                    try:
                        j = self.stashd_inf.getrawtransaction(path[1], 1, skip_cache=self.skip_cache)
                        return j
                    except Exception as e:
                        raise
                else:
    ...
    def get_block_hash(self, block_number):
        return self.stashd_inf.getblockhash(block_number)

    def current_height(self):
        return self.stashd_inf.getheight()

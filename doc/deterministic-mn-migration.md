...
to the SMT functionalities and it focuses on the migration from
...
[Stash 0.13 Upgrade Procedure](https://docs.stashpay.io/en/stable/masternodes/dip3-upgrade.html#masternode-registration-from-smt),
which I highly recommend.

It's worth noting here, that this process can be performed completely
independently from SMT (using the Stash Core wallet), however, the
inclusion of this function in SMT is aimed at the maximum facilitation
of this process.
...
### Contents
  ...
  * [DMN registration methods](#dmn-registration-methods) 
    ...
    * [Manual method using own Stash node](#manual-method-using-own-stash-node-m3)
  * [FAQ](#faq)

### Glossary
**DIP3**: Stash blockchain parameter, enabling the Deterministic
...
**Spork 15**: a parameter causing the Stash network to switch from the
...
DML, but the final decision will be made by the _Stash Core Group_
...
**RPC node**: a Stash node providing an interface for external apps to
communicate with the Stash network. StashMasternodeTool (**SMT**) requires
...
information kept in SMT. In the screenshot below, the fields important
...
- Check if the collateral address and the related BIP32 path are
  correct. Verification of this in SMT:  
...
- Close all other programs that could communicate with the same hardware
  wallet device which you use in SMT (eg. Trezor online wallet). Two (or
...
* [Manual method using own Stash node (M3)](#manual-method-using-own-stash-node-m3)

In the automatic method, the majority of activities are performed by SMT
...
the Stash Core debug console and copying the results back to SMT, so it
...
you :
- If you have your own Stash Core synchronized with the network with 
  enabled RPC and indexing, choose either
  [Automatic method using own RPC node](#automatic-method-using-own-rpc-node-m2)
  or
  [Manual method using own Stash node](#manual-method-using-own-stash-node-m3).
- If you have your own Stash Core synchronized with the network but
  without the RPC interface or indexing enabled, choose
  [Manual method using own Stash node](#manual-method-using-own-stash-node-m3).
- If you do not have and you can not easily run Stash Core (for example,
...
([details](https://github.com/stashpayio/dips/blob/master/dip-0003.md#registering-a-masternode-proregtx)).
If this raises your concerns (eg. you do not trust the RPC node
operator), choose another method.

>  **Note 1.** The RPC nodes preconfigured in SMT (alice, luna, suzy)
>  are managed by the author of the SMT application (ie by me). For my
>  part, I can ensure that no sensitive information (such as private
>  keys) is logged anywhere. All parameters are passed to the Stash
...
>  [ProUpRegTx](https://github.com/stashpayio/dips/blob/master/dip-0003.md#updating-registrar-of-masternode-proupregtx)
...
>  address and owner key using data available on the Stash network. This
>  feature is implemented in SMT under the **Get status** button. If
...
You do this in the main SMT window.
...
> button is still not visible, make SMT to establish the connection with
> the Stash network to force it to read the current network parameters.
> You can do this by clicking the **Check Stash Network Connection**
> toolbar button.
...
Click the option **Remote Stash RPC Node (automatic method)**
...
At this step, the wizard executes two operations on the remote Stash node
...
configuration in SMT, nevertheless it's worth saving them in a separate
file.

The last thing to do here depends on your use case:
* **UC1**: as suggested on the screen, copy the `masternodeblsprivkey =
  ...` line into the stash.conf file on the server running your
  masternode, then restart the stashd daemon..
...
RPC node to interact with the Stash network. Its advantage is that the
...
your own Stash full node with enabled RPC interface and the address
...
Remember to configure a connection between SMT and your RPC node and to
turn off all others.

> Note, that the RPC node has nothing to do with your masternode - this
> may be your local Stash Core instance, and that's actually the most
...
## Manual method using own Stash node (M3)
...
Stash Core wallet, not necessarily with the RPC interface and indexing
enabled. The disadvantage is the need to manually execute some commands
from the Stash Core debug console, which may not be very comfortable for
some people.

#### Step 1. Install Stash Core and synchronize it with the network
...
Click the option **Your own Stash-Qt wallet (manual method)**
...
#### Step 7. Copy the command text generated by SMT in field "2"  
...
#### Step 8. Execute the command in the Stash Core debug console
...
#### Step 10. Paste the command output into field "3" (SMT) and click _Sign message with hardware wallet_  
...
#### Step 12. Execute the command in the Stash Core debug console
...
#### Step 14. Paste the copied transaction number into field "5" in SMT.
...
from Stash Core - as of now, SMT does not have a user interface
supporting this functionality, so you will need to manually execute
commands from the Stash Core Debug console.

**Here are the steps**
1. Start Stash Core wallet and wait for it to synchronize with the
   network

2. Import the owner private key to the Stash Core wallet if you have not done it
   before
    ...
    2.2. Copy the owner private key to clipboard (eg. from SMT
    configuration).
...
3. Send an update registrar transaction (**ProUpRegTx**)
    ...
    3.1. Find an address in Stash Core you are going to cover transaction 
    ...
    **Stash address**, using the context menu associated with the "eye" 
    button on the right, then copy the Stash address from the corresponding 
    edit box
...
2. Send an update registrar transaction (**ProUpServTx**)
    ...
    2.1. Find an address in Stash Core you are going to cover transaction
    ...
    **feeSourceAddress** parameter in next steps. To list addresses
    ...
    2.3. Execute the update service command from the Stash Core Debug
    console  
